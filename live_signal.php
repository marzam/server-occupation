...
//	exit;
	...
	//	echo json_encode($row);
		...
		array_push($myArray, array("y" => $row["signal"], "x" => $row["mac"]));
...
$dataPoints = get_value($conn, "select mac, avg(device_signal) as 'signal' from tbNetworkRecord_v2 where date_time < current_timestamp and date_time > date_add(current_timestamp, interval -" . $minutos . " minute) and substring(pack_type,3,2) != '40' group by mac;");
#$dataPoints = get_value($conn, "select id_addr, avg(device_signal) as 'signal' from tbNetworkRecord where date_time < current_timestamp and date_time > date_add(current_timestamp, interval -" . $minutos . " minute) and substring(pack_type,3,2) != '40' group by id_addr;");
