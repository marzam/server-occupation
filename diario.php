...
//	exit;
	...
	//	echo json_encode($row);
		...
		array_push($myArray, array("y" => $row["size"], "x" => strtotime($date . $row["hour"] . ":00:00")*1000));
...
$date = htmlspecialchars($_GET["date"]);
$dataPoints = get_value($conn, "select hour(date_time) as 'hour', count(distinct id_addr) as 'size' from tbNetworkRecord where date(date_time) = '" . $date . "' and substring(pack_type,3,2) != '40' group by hour(date_time);");
